...
* The plate SVG in the zip folders are however, built off of the layer from the actual kiCAD design files; it is what I used when I had my plate cut. 
* _IMPORTANT NOTE:_ If you use the swillkb.com tool, I recommend printing it out on paper before hand to make sure that all of the holes line up. The concern is that the space between the main board and the numpad may be off in the KLE layout. While I've double checked the KLE layout dimensions for accuracy, nothing beats the paper test! 
* OTHER NOTES:
